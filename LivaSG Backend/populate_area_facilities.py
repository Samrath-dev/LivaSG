...
async def main():
    ...
    db_path = os.path.join(BASE_DIR, args.db_name)
        ...
        try:
            ...
            cur.execute(
                ...
                INSERT INTO planning_area_facilities (
                    area_name, schools, sports, hawkers, healthcare, greenSpaces, carparks, transit, calculated_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))
                ...
                (
                    ...
                    counts["healthcare"], counts["greenSpaces"], counts["carparks"], counts["transit"],
                ),
            )
            inserted += 1
